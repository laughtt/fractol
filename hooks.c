...
/*   Updated: 2019/03/22 11:43:19 by jcarpio-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			mouse_move(int x, int y, t_fract *frac)
{
	if (frac->numbf == 2)
	{
		if (x > 0 && x < IMG_HEIGHT && y > 0 && y < IMG_WIDTH)
		{
			...
			frac->xx = ((float)(x - IMG_HEIGHT / 2) / (float)(-IMG_HEIGHT / 2 \
						- IMG_HEIGHT / 2)) * (-2.0f) + 1.0f;
			frac->yy = ((float)(y - IMG_WIDTH / 2) / (float)(-IMG_WIDTH / 2 \
						- IMG_WIDTH / 2)) * (-2.0f) + 1.0f;
			ft_bzero(frac->image->data, IMG_WIDTH *\
					IMG_HEIGHT * frac->image->bpp);
			ft_draw_fractal(frac);
		}
	}
	return (0);
